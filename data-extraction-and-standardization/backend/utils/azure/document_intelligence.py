...
def extract_figure(s3_client, poller_result, parent_file, pdf_raw_data):
    ...
    for d in poller_result['figures']:
        try:
            ...
            trace.append(public_url) if public_url!=-1 else trace.append(f'Error while processing : {d}')
        except:
            trace.append(f'Error while processing : {d}')
    return trace 
...
def extract_tables(s3_client, poller_result, parent_file):
    ...
    for table in poller_result['tables']:
        try:
            ...
            trace.append(public_url) if public_url!=-1 else trace.append(f'Error while processing : {table}')
        except:
            trace.append(f'Error while processing : {table}')
    return trace
